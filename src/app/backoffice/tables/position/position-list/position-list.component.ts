...
export class PositionListComponent implements OnInit, OnDestroy, EntityList<Position> {
  ...
  handleLoadCount() {
    this.store.dispatch(positionActions.count(addActionId({})));
  }
  ...
  onLazyLoadEvent(event: TableLazyLoadEvent) {
    this.store.dispatch(positionActions.loadAll(addActionId({ payload: event })));
  }
    ...
    switch (event.type) {
      ...
      case TableEventType.DELETE: {
        this.confirmationSrv.confirm({
          ...
          accept: () => {
            this.store.dispatch(positionActions.delete(addActionId({ id: event.value.id })));
          },
        });
